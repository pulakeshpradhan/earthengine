...
```mermaid
flowchart TD
    A["Reducers"] --> B["📅 Temporal Reduction\n(Many Images → One Image)"]
    A --> C["📐 Spatial Reduction\n(Many Pixels → One Number)"]
...
flowchart LR
    ...
    B -->|"Many images → one image"| C["collection.median()\ncollection.mean()"]
    B -->|"Pixels in area → stats"| D["image.reduceRegion()"]
    B -->|"Multiple regions"| E["image.reduceRegions()"]
    B -->|"Images + time → chart"| F["ui.Chart.image.series()"]
